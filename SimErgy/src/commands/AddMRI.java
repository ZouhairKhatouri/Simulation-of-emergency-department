...
/**
 * 
 * This command adds a MRI service to the CLUI simulation system on the ed (repaired by the name inputed by the user) 
 * and with a service-time law also inputed by the user.
 *
 */

public class AddMRI extends Command{
	
	private String edName;
	private ProbabilityDensity servicetime;

	public AddMRI(SimErgyCLUI clui,String[] args) throws ArgumentsMismatchException, SyntaxErrorException {
	...
	public void execute() {
		if(getClui().edMap.containsKey(edName)) {
			...
			if(edepartment.mriDepartment==null) {
				int ID =edepartment.nextID();
				MRI mriDepartement = new MRI(ID,servicetime);
				edepartment.mriDepartment = mriDepartement;
