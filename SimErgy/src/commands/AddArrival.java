...
/**
 * 
 * This command adds a Arrival service to the CLUI simulation system on the ed (repaired by the name inputed by the user) 
 * and with a service-time law also inputed by the user.
 *
 */

public class AddArrival extends Command{
	
	private String edName;
	private ProbabilityDensity servicetime;

	public AddArrival(SimErgyCLUI clui,String[] args) throws ArgumentsMismatchException, SyntaxErrorException {
	...
	public void execute() {
		if(getClui().edMap.containsKey(edName)) {
			...
			if(edepartment.arrival==null) {
					int ID = edepartment.nextID();
					Arrival arrivalDepartement = new Arrival(ID,servicetime);
					edepartment.arrival = arrivalDepartement;
