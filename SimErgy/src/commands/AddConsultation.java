...
/**
 * 
 * This command adds a Consultation service to the CLUI simulation system on the ed (repaired by the name inputed by the user) 
 * and with a service-time law also inputed by the user.
 *
 */

public class AddConsultation extends Command{
	
	private String edName;
	private ProbabilityDensity servicetime;

	public AddConsultation(SimErgyCLUI clui,String[] args) throws ArgumentsMismatchException, SyntaxErrorException {
	...
	public void execute() {
		if(getClui().edMap.containsKey(edName)) {
			...
			if(edepartment.consultationDepartment==null) {
				int ID = edepartment.nextID();
				Consultation consultationDepartement = new Consultation(ID,servicetime);
				edepartment.consultationDepartment = consultationDepartement;
