...
/**
 * 
 * This command adds a Blood-test service to the CLUI simulation system on the ed (repaired by the name inputed by the user) 
 * and with a service-time law also inputed by the user.
 *
 */

public class AddBloodTest extends Command {

	private String edName;
	private ProbabilityDensity servicetime;

	public AddBloodTest(SimErgyCLUI clui, String[] args) throws ArgumentsMismatchException, SyntaxErrorException {
	...
	public void execute() {
		if(getClui().edMap.containsKey(edName)) {
			...
			if(edepartment.bloodTestDepartment==null) {
				int ID = edepartment.nextID();
				BloodTest bloodTestDepartement = new BloodTest(ID,servicetime);
				edepartment.bloodTestDepartment = bloodTestDepartement;
